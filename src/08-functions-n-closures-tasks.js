...
function getPolynom(...arg) {
  const argArr = arg.reverse();
  return (x) => argArr.reduce((prev, cur, ind) => prev + cur * (x ** ind), 0);
}
...
function memoize(func) {
  ...
  return (x) => {
    if (!resMap.has(x)) resMap.set(x, func(x));
    return resMap.get(x);
  };
}
...
function logger(func, logFunc) {
  return (...x) => {
    const args = [...x].map((i) => JSON.stringify(i)).join(',');
